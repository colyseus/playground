...
export function InspectConnection({
	...
	const hasState = (connection.state !== null);

	const allowReconnect = !connection.connection.isOpen;
	...
	const reconnect = () => {
		...
		client.reconnect(connection.reconnectionToken);
	}
	...
	const sendMessage = () => {
		connection.send(messageType, JSON.parse(message));
	};

	useEffect(() => {
		const unbindOnMessage = connection.onMessage("*", (type, message) => {
			setAllMessages([...allMessages, { type, message, in: true }]);
		});
	...
	<>
		<div>
				<div className="flex items-center">
					...
					<div className="flex mt-2">
						<textarea name="message" id="message" className="border border-gray-300 w-80 font-mono p-1.5 rounded" rows={1} onChange={handleMessageChange} value={message} />
					</div>
			...
			<div className="grid grid-cols-3 gap-2 my-2">
				<button className="bg-green-500 enabled:hover:bg-green-600 disabled:cursor-not-allowed text-white font-bold py-2 px-4 rounded disabled:opacity-50" disabled={!allowReconnect} onClick={reconnect}>
				...
				<button className="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded" onClick={drop}>
				...
				<button className="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded" onClick={leave}>
			...
			{/* Messages */}
			<table className="w-full border-slate-300 text-center text-xs">
				<thead>
					<tr>
						<th>&nbsp;</th>
				...
				{(allMessages).map((message, i) => (
					<tr key={i} className={"border-b " + (message.in ? "bg-red-100" : "bg-green-100")}>
						<td className="p-2">
							{message.in &&
								<svg className="w-3 inline text-red-500" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path d="M169.4 470.6c12.5 12.5 32.8 12.5 45.3 0l160-160c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L224 370.8 224 64c0-17.7-14.3-32-32-32s-32 14.3-32 32l0 306.7L54.6 265.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3l160 160z"/></svg>}

							{message.out &&
								<svg className="w-3 inline text-green-500" fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512"><path d="M214.6 41.4c-12.5-12.5-32.8-12.5-45.3 0l-160 160c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L160 141.2V448c0 17.7 14.3 32 32 32s32-14.3 32-32V141.2L329.4 246.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3l-160-160z"/></svg>}
						</td>
						...
						<td className="w-full p-2 text-left">
