...
import { Connection, roomsBySessionId, messageTypesByRoom, messageSchemasByRoom } from "../utils/Types";
...
import { MessageSchemasView } from "./MessageSchemasView";
...
export function InspectConnection({
	...
	const messageSchemasByType = messageSchemasByRoom[room.name]
	...
	const [messageSchemas, setMessageSchemas] = useState<MessageSchemas>(messageSchemasByType?.[messageTypes[0]])
    ...
    useEffect(() => {
        const schemas = messageSchemasByType?.[messageType]

        setMessageSchemas(schemas)
        setMessage(JSON.stringify(schemas?.client || {}, null, 2))
    }, [messageType])
