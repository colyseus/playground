...
export function InspectConnection({
	...
	// 		setAllMessages([{ type, message, in: true, now: performance.now() }, ...allMessages]);
	// 	});
		...
		<div>
				<div className="flex items-center">
					...
					<div className="flex mt-2 grow pr-2">
						<textarea name="message" id="message" className="border border-gray-300 w-80 font-mono p-1.5 rounded w-full" rows={1} onChange={handleMessageChange} value={message} />
					</div>
					...
					<table className="table-auto w-full border-collapse text-center text-xs border-t border-l border-r">
						<thead>
							<tr className="border-b">
								...
								<th className="p-3 w-full">Payload</th>
							</tr>
						</thead>
						...
						{(allMessages).map((message, i) => (
							<tr key={connection.sessionId + "-" + message.now} className={"border-b " + (message.in ? "bg-red-100" : "bg-green-100")}>
								<td className="p-2">
									{message.in &&
